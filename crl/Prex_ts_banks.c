...
lefthrs=0; righthrs=0;counthouse=0;
events_per_read=1;  
 ...
 if (adc18bit) { 
     numadc18 = adc18_getNum(); 
      ...
      for (iadc = 0; iadc < numadc18; iadc++) { 
         ...
         adc18_intgain(iadc,int_gain[iadc]);       
         adc18_setconv(iadc,conv_gain[iadc]);    
...
if ( !mtread )  {
ncnt++;
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x102 ) << 16) | ( 
0x01 ) << 8) | ( events_per_read );	((rol->dabufp))++; ; if(flexio==1) {
...
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ; /* # 934 "Prex_ts_banks.c" */
set_tir_oport(0x02 );
if(noscaler == 0) {
  if(scaler2==1) {
    nscaler=2;
...
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x105 ) << 16) | ( 
0x01 ) << 8) | ( events_per_read );	((rol->dabufp))++; ;   for (i = 0; i <nscaler; i++){
    for (j = 0; j < 32; j++) {
...
& 1) != 0) { (rol->dabufp) = ((long *)((char *) (rol->dabufp))+1); *StartOfBank += 1; }; if 
((*StartOfBank & 2) !=0) { *StartOfBank = *StartOfBank + 2; (rol->dabufp) = ((long *)((short *) 
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ; /* # 985 "Prex_ts_banks.c" */
/* # 996 "Prex_ts_banks.c" */
/* # 1007 "Prex_ts_banks.c" */
 }
 unset_tir_oport(0x02 );
 if (adc18bit) {   
   set_tir_oport(0x01 );
...
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x110 ) << 16) | ( 
0x01 ) << 8) | ( events_per_read );	((rol->dabufp))++; ;        *rol->dabufp++ = tirdata & 0xfff;
       *rol->dabufp++ = iadc;
       *rol->dabufp++ = maxloop;
       *rol->dabufp++ = icnt;
       *rol->dabufp++ = csrA;
...
& 1) != 0) { (rol->dabufp) = ((long *)((char *) (rol->dabufp))+1); *StartOfBank += 1; }; if 
((*StartOfBank & 2) !=0) { *StartOfBank = *StartOfBank + 2; (rol->dabufp) = ((long *)((short *) 
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ; cont1: 
     nevt = adc18_getevtcnt(iadc);
   }             
   unset_tir_oport(0x01 );
 }   
 if (adc16bit) {
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x114 ) << 16) | ( 
0x01 ) << 8) | ( events_per_read );	((rol->dabufp))++; ;    for (i=0; i < ADC_NUMSLOTS; i++) rol->dabufp = readoutHAPADC(rol->dabufp,i);
*StartOfBank = (long) (((char *) (rol->dabufp)) - ((char *) StartOfBank));	if ((*StartOfBank 
& 1) != 0) { (rol->dabufp) = ((long *)((char *) (rol->dabufp))+1); *StartOfBank += 1; }; if 
((*StartOfBank & 2) !=0) { *StartOfBank = *StartOfBank + 2; (rol->dabufp) = ((long *)((short *) 
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ;  }
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x112 ) << 16) | ( 
0x01 ) << 8) | ( events_per_read );	((rol->dabufp))++; ; *rol->dabufp++ = getDataHAPTB();
...
& 1) != 0) { (rol->dabufp) = ((long *)((char *) (rol->dabufp))+1); *StartOfBank += 1; }; if 
((*StartOfBank & 2) !=0) { *StartOfBank = *StartOfBank + 2; (rol->dabufp) = ((long *)((short *) 
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ; if(bmwscan==1) {
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x113 ) << 16) | ( 
0x01 ) << 8) | ( events_per_read );	((rol->dabufp))++; ;   *rol->dabufp++ = getCleanSCAN();
...
if(( READOUT_VQWK == 1) ) {
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x111 ) << 16) | ( 
0x01 ) << 8) | ( events_per_read );	((rol->dabufp))++; ; { 
 int id = 0;
 if (firstEvents>1){
   firstEvents--;
