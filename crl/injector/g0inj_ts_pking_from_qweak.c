...
void g0inj_ts_pking_from_qweak__init (rolp)
     rolParam rolp;
{
...
/* # 7 "g0inj_ts_pking_from_qweak.c" 2 */
...
/* # 8 "g0inj_ts_pking_from_qweak.c" 2 */
...
/* # 24 "g0inj_ts_pking_from_qweak.c" 2 */
/* # 1 "g0inj.h" 1 */
/* # 1 "usrstrutils.c" 1 */
...
->list) )->l = 0; }} ;} ; if(__user_event__ == (DANODE *) 0) { logMsg ("TRIG ERROR: no pool buffer 
...
StartOfUEvent = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x85 ) << 16) | ( 0x10 ) << 8) | (0xff & rol->pid 
  ...
  len = (slen+3)/4;
  *(rol->dabufp)++ = len+1;
*(rol->dabufp)++ = (unsigned long)(((( 0 )&0xffff)<<16)|(( slen )&0xff) | ((( 3 ) & 
0xff)<<8)); ;   while(len--) *(rol->dabufp)++ = *sptr++;
  sptr = (unsigned long *) rol->usrString;
  slen = strlen((char *) sptr);
  len = (slen+3)/4;
  *(rol->dabufp)++ = len+1;
*(rol->dabufp)++ = (unsigned long)(((( 1 )&0xffff)<<16)|(( slen )&0xff) | ((( 3 ) & 
0xff)<<8)); ;   while(len--) *(rol->dabufp)++ = *sptr++;
  sptr = (unsigned long *) file_configusrstr;
  slen = strlen((char *) sptr);
  len = (slen+3)/4;
  *(rol->dabufp)++ = len+1;
*(rol->dabufp)++ = (unsigned long)(((( 2 )&0xffff)<<16)|(( slen )&0xff) | ((( 3 ) & 
0xff)<<8)); ;   while(len--) *(rol->dabufp)++ = *sptr++;
...
)->l->n = ( __user_event__ );( &(rol->output->list) )->l = ( __user_event__ );} ( __user_event__ 
)->n = 0;( &(rol->output->list) )->c++;	if(( &(rol->output->list) )->add_cmd != ((void *) 0) ) 
(*(( &(rol->output->list) )->add_cmd)) (( &(rol->output->list) )); } ; } else { { if (( 
__user_event__ )->part == 0) { free( __user_event__ ); __user_event__ = 0; } else { {if(! ( & __user_event__ 
->part->list )->c ){( & __user_event__ ->part->list )->f = ( & __user_event__ ->part->list )->l = ( 
__user_event__ );( __user_event__ )->p = 0;} else {( __user_event__ )->p = ( & __user_event__ ->part->list 
)->l;( & __user_event__ ->part->list )->l->n = ( __user_event__ );( & __user_event__ ->part->list 
)->l = ( __user_event__ );} ( __user_event__ )->n = 0;( & __user_event__ ->part->list 
)->c++;	if(( & __user_event__ ->part->list )->add_cmd != ((void *) 0) ) (*(( & __user_event__ 
->part->list )->add_cmd)) (( & __user_event__ ->part->list )); } ; } if( __user_event__ ->part->free_cmd 
!= ((void *) 0) ) { (*( __user_event__ ->part->free_cmd)) ( __user_event__ 
->part->clientData); } } ;	} __user_event__ = (DANODE *) 0; }; ; }
/* # 22 "g0inj.h" 2 */
/* # 1 "myevtypes.h" 1 */
/* # 24 "g0inj.h" 2 */
...
/* # 25 "g0inj_ts_pking_from_qweak.c" 2 */
int VQWK_DATA_READY_TIME = 20;
struct vme_ts2 *tsP;
unsigned long laddr;
...
16) | (( 0x10 ) << 8) | (0xff & *(rol->nevents));	}	((rol->dabufp))++;} ; if(( event_ty == 1) ) {
if(( READOUT_BEAM_SCALERS == 1) ) {
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x3101 ) << 16) | 
( 0x01 ) << 8) | ( 0 );	((rol->dabufp))++; ; { 
        ...
        *rol->dabufp++ = 0;
        ...
        {
          *(rol->dabufp)++ = header;
real_num_of_words = sis3801ReadOneModule(id,num_of_words, rol->dabufp);           rol->dabufp--;
...
if(( READOUT_VQWK == 1) ) {
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x3102 ) << 16) | 
( 0x01 ) << 8) | ( 0 );	((rol->dabufp))++; ; { 
        id=0;
        while (id < 21){
          timer[id++] = 0;
...
((*StartOfBank & 1) != 0) { (rol->dabufp) = ((long *)((char *) (rol->dabufp))+1); *StartOfBank += 1; }; if 
((*StartOfBank & 2) !=0) { *StartOfBank = *StartOfBank + 2; (rol->dabufp) = ((long *)((short *) 
(rol->dabufp))+1);; };	*StartOfBank = ( (*StartOfBank) >> 2) - 1;}; ; {	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x3112 ) << 16) | 
( 0x01 ) << 8) | ( 0 );	((rol->dabufp))++; ; { 
        int id = 0;
        *rol->dabufp++ = 'WARN';
...
{	long *StartOfBank; StartOfBank = (rol->dabufp); *(++(rol->dabufp)) = ((( 0x3103 ) << 16) | 
( 0x01 ) << 8) | ( 0 );	((rol->dabufp))++; ; { 
      int id=0;
