...
void AccumAndTriggered__init (rolp)
     rolParam rolp;
{
...
( 0x01 ) << 8) | ( 0 );	((rol->dabufp))++; ; { 
  ...
  }else if (current_mode ==1 && event_counter>= 1000 ){
     current_mode=0;
     event_counter=0;
     tir[1]->tir_oport=0xffff;
     sis3320StopDelay(0,0 );
