...
THaEtClient::THaEtClient(int smode)
{
  ...
  initflags();
  const char* defaultcomputer = ADAQ3;
...
THaEtClient::~THaEtClient() {
  ...
  delete [] etfile;
  fStatus = codaClose();
  if (fStatus == CODA_ERROR) cout << "ERROR: closing THaEtClient"<<endl;
};
...
int THaEtClient::init(const char* mystation) 
{
  ...
  if ((fStatus = et_station_create(id, &my_stat, station, sconfig)) < ET_OK) {
    if (fStatus == ET_ERROR_EXISTS) {        
      // ok 
    }
    else if (fStatus == ET_ERROR_TOOMANY) {
      cout << "THaEtClient: too many stations created"<<endl;
      fStatus = CODA_ERROR;
      return fStatus;
    }
    else if (fStatus == ET_ERROR_REMOTE) {
      cout << "THaEtClient: memory or improper arg problems"<<endl;
      fStatus = CODA_ERROR;
      return fStatus;
    }
    else if (fStatus == ET_ERROR_READ) {
      cout << "THaEtClient: network reading problem"<<endl;
      fStatus = CODA_ERROR;
      return fStatus;
    }
    else if (fStatus == ET_ERROR_WRITE) {
...
// reset 
    ...
    for (j=0; j < nread; j++) {
      ...
      if (swapflg == ET_SWAP) {            
        printf("error: ET_SWAP needed ?  Well, et_event_CODAswap is deprecated, so you'll need to use EVIO's evioswap(uint32_t *buf, int toLocal, unint32_t *dest)\n");
