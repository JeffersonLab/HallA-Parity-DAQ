...
void setIgnoreRun(int flag){// 1 to ignore runs, 0 to not ignore.
    IgnoreRuns = flag;
}

int getIgnoreRun(){        // 1 to ignore runs, 0 to not ignore.
    return IgnoreRuns;
}

void setRunFlag(int flag){  // 1 for running, 0 for not running.
    Running = flag;
}

int getRunFlag(){          // 1 for running, 0 for not running.
    return Running;
}
...
STATUS cfSockSer (void)
{
    ...
    while(closeSocket==0) {
        ...
        if ((newFd = accept (cf_sFd, (struct sockaddr *) &clientAddr, &sockAddrSize)) == SOCK_ERROR) {
            perror ("accept");
            return (SOCK_ERROR);
        }
...
void cfSockWorkTask(int task_sFd,       /* server's socket fd */
...
u_short port)                           /* client's socket port */
{
    ...
    while ((nRead = fioRead (task_sFd, (char *) &clientRequest, sizeof (clientRequest))) > 0) {
        ...
        if (getRunFlag()==0 || getIgnoreRun()==1) {
            ...
            if (clientRequest.command_type==COMMAND_HAPTB) {
                ...
                if (taskSpawn(workName, SERVER_WORK_PRIORITY, 0, SERVER_STACK_SIZE, taskHAPTB_CF,
                clientRequest.command,
                clientRequest.par1,
                clientRequest.par2,
                clientRequest.par3,
